        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '8px 0' }}>
          ...
          {!loading && filtered.length === 0 && (
            <p style={{ textAlign: 'center', padding: '30px', color: '#8E8E93', fontSize: '13px' }}>
              {search ? 'Nessun risultato.' : (mode === 'followers' ? 'Nessun follower ancora.' : 'Non segue nessuno.')}
            </p>
          )}
          {!loading && filtered.map(u => {
            const username = u.username || 'utente'
            const avatarBg = '#7CA982'
            return (
              <button
                key={u.id}
              ...
              >
                <div
                  style={{
                    ...
                    height: '40px',
                    borderRadius: '50%',
                    background: u.avatar_url ? 'transparent' : avatarBg,
                    color: '#FFF',
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'center',
